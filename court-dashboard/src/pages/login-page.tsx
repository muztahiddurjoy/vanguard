...
export function LoginPage() {
  ...
  const lookUp = async (id: string): Promise<CourtStaff> => {
    if (!apiEnabled()) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.success(t.login.welcome(pickName(staff)))
  ...
  const points = [
    { Icon: CalendarDays, text: t.login.point1 },
    { Icon: Fingerprint, text: t.login.point2 },
    { Icon: PhoneCall, text: t.login.point3(f.plain(Number(HELPLINE_NUMBER))) },
  ]
  const demos = [DEMO_STAFF.magistrate, DEMO_STAFF.tribunal].map((id) => findStaff(id)!)
  ...
  return (
    <div className="grid min-h-svh lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
      <aside className="hidden flex-col justify-between gap-10 bg-sidebar p-10 text-sidebar-foreground lg:flex xl:p-14">
        ...
        <p className="text-sm text-sidebar-foreground/70">{t.app.system}</p>
      </aside>
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              {demos.map((staff) => (
                <Button
                  ...
                  onClick={() => fillDemo(staff.id)}
                >
                  <UserRound aria-hidden data-icon="inline-start" />
                  {t.login.demoAccount(pickName(staff), pickName(staff.court))}
                </Button>
              ))}
            </div>
          </section>
