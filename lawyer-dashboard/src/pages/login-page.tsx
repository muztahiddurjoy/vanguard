...
import { DEMO_LAWYERS, DEMO_PASSWORD, MIN_PASSWORD_LENGTH, findLawyer } from "@/data/lawyers"
...
export function LoginPage() {
  ...
  // Problems only the lookup can find: an ID not on the panel, or no server.
  ...
  const validate = (): Errors => {
    const errors: Errors = {}
    if (!lawyerId.trim()) errors.id = t.login.errors.idRequired
  ...
  const lookUp = async (id: string): Promise<Lawyer> => {
    if (!apiEnabled()) {
      const found = findLawyer(id)
      if (!found) throw new ApiError(401, "not on the panel")
      return found
    }
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setAttempted(true)
    setIdProblem(null)
    if (!lawyerId.trim()) return idRef.current?.focus()
    if (!password || password.length < MIN_PASSWORD_LENGTH) return pwRef.current?.focus()
    setSubmitting(true)
    try {
      const lawyer = await lookUp(lawyerId)
      signIn(lawyer, remember)
      toast.success(t.login.welcome(pick(lawyer.name)))
  ...
  const points = [
    { Icon: BriefcaseBusiness, text: t.login.point1 },
    { Icon: Send, text: t.login.point2 },
    { Icon: CalendarDays, text: t.login.point3 },
  ]
  const demos = [
      ...
      <aside className="hidden flex-col justify-between gap-10 bg-sidebar p-10 text-sidebar-foreground lg:flex xl:p-14">
        <div className="flex items-center gap-3">
          <span className="flex size-11 items-center justify-center rounded-lg bg-sidebar-primary text-sidebar-primary-foreground">
            <BriefcaseBusiness aria-hidden className="size-6" />
          </span>
        ...
        <p className="text-sm text-sidebar-foreground/70">{t.app.office}</p>
      </aside>
