...
const KEY = "dlas.prison.session"

// "Keep me signed in" uses localStorage; otherwise the session ends with the tab.
function readSession(): JailStaff | null {
  for (const store of [() => window.localStorage, () => window.sessionStorage]) {
    try {
      const raw = store().getItem(KEY)
      if (raw) return JSON.parse(raw) as JailStaff
    } catch {
...
function writeSession(user: JailStaff | null, remember: boolean) {
  try {
...
export function AuthProvider({
  ...
  initialUser?: JailStaff | null
}) {
  const [user, setUser] = useState<JailStaff | null>(() =>
    initialUser === undefined ? readSession() : initialUser,
  )

  const signIn = useCallback((staff: JailStaff, remember: boolean) => {
    setUser(staff)
