/** English plurals: counts arrive formatted, and English writes 1 as "1". */
const count = (n: string, one: string, many: string) => `${n} ${n === "1" ? one : many}`

// English is the source of truth for the message shape; bn.ts must match it.
// Function messages receive values that are already locale-formatted.
//
// Writing style: plain words a first-day officer can follow.
// Keep the spec's named terms (e.g. "Pending AI Triage") but always pair them
// with a one-line explanation of what they mean.
export const en = {
  app: {
    title: "District Legal Aid Office",
    system: "Digital Legal Aid System",
    district: (d: string) => `${d} District`,
    skipToContent: "Skip to main content",
  },
  nav: {
    label: "Main navigation",
    home: "Home",
    queue: "Work queue",
    cases: "All cases",
    lawyers: "Lawyers",
    hearings: "Hearings",
    reports: "Reports",
    help: "Help",
    settings: "Settings",
    toggleSidebar: "Show or hide the menu",
  },
  common: {
    loading: "Loading",
    backHome: "Go to Home",
    notAvailable: "This feature is not available yet.",
  },
  login: {
    title: "Sign in",
    subtitle: "Use your officer ID and password to open the dashboard.",
    heroTitle: "Legal aid, faster and safer",
    heroBody:
      "One place for district legal aid officers to see every case that needs them and act on it.",
    point1: "See every case that needs you in one simple list",
    point2: "Check AI suggestions — you always make the final decision",
    point3: "Safety checks stop calls that could put someone at risk",
    demoTitle: "Demo account",
    demoBody: "Sign in as a District Legal Aid Officer to explore the dashboard.",
    useDemo: "Fill in the demo account",
    id: "Officer ID or email",
    idHint: "Example: DLAO-RGP-0142",
    password: "Password",
    showPassword: "Show password",
    hidePassword: "Hide password",
    remember: "Keep me signed in on this computer",
    rememberHint: "Only tick this on a computer nobody else uses.",
    submit: "Sign in",
    submitting: "Signing in…",
    forgot: "Forgot your password?",
    forgotTitle: "Reset your password",
    forgotBody:
      "For safety, passwords are reset by your district's system administrator. Call the office helpdesk or ask your supervisor, and you will get a one-time password by SMS.",
    forgotOk: "OK, got it",
    errors: {
      idRequired: "Enter your officer ID or email.",
      passwordRequired: "Enter your password.",
      passwordShort: (n: string) => `The password must be at least ${n} characters.`,
    },
    welcome: (name: string) => `Welcome, ${name}`,
    signedOut: "You have signed out.",
  },
  notFound: {
    title: "Page not found",
    body: "This page does not exist. Use the menu, or go back to Home.",
  },
  notifications: {
    title: "Notifications",
    label: (n: string) => `Notifications: ${n} unread`,
    markAll: "Mark all as read",
    empty: "You are all caught up.",
    hearingToday: (time: string, place: string) => `Hearing today at ${time}, ${place}`,
    openQueue: "Open the work queue",
    unread: "Unread",
  },
  header: {
    language: "Language",
    accountMenu: "Account menu",
    profile: "My profile",
    settings: "Settings",
    signOut: "Sign out",
  },
  home: {
    greeting: {
      morning: (name: string) => `Good morning, ${name}`,
      afternoon: (name: string) => `Good afternoon, ${name}`,
      evening: (name: string) => `Good evening, ${name}`,
    },
    intro: (date: string) => `${date}. Here is what needs your attention today.`,
    startHere: "Start here",
    startHereHint: "The most urgent cases that need you now, in order.",
    allClear: "Nothing urgent right now.",
    seeQueue: "Open the full work queue",
    queuesTitle: "Your lists",
    queuesHint: "Each list groups cases that need the same kind of work.",
    comingUp: "Coming up",
    comingUpHint: "Court hearings and mediation meetings in the next two weeks.",
    seeHearings: "See all hearings",
  },
  cases: {
    title: "All cases",
    description:
      "Every case in this office, open and closed. Search for a person or case number, or filter the list.",
    status: "Status",
    statusOpen: "Open",
    statusClosed: "Closed",
    statusAll: "All",
    type: "Type of case",
    anyType: "All types",
    caption: "All cases in the office",
    columns: {
      applicant: "Applicant",
      type: "Type",
      status: "Status",
      received: "Received",
      lawyer: "Lawyer",
      action: "Action",
    },
    outcome: {
      resolved: "Resolved",
      settled: "Settled by mediation",
      withdrawn: "Withdrawn",
      referred: "Referred elsewhere",
    },
    closedOn: (date: string) => `Closed ${date}`,
    open: "Open case",
    none: "No lawyer",
    count: (n: string, total: string) => `Showing ${n} of ${total} cases`,
    empty: "No cases match. Try a different search or filter.",
  },
  lawyers: {
    title: "Lawyers",
    description:
      "The district's panel lawyers who take legal aid cases. See who is keeping up, and remind anyone who has stopped reporting.",
    summary: (n: string, late: string) => `${n} lawyers on the panel · Reminders needed: ${late}`,
    since: (year: string) => `On the panel since ${year}`,
    active: "Open cases",
    lastUpdate: "Last progress update",
    never: "No updates yet",
    status: {
      ok: "Up to date",
      free: "Available for new cases",
      late: (n: string) => `Missed ${count(n, "update", "updates")}`,
      reminded: "Reminder sent — waiting for a reply",
    },
    theirCases: "Their open cases",
    noCases: "No open cases right now.",
    remind: "Send reminder",
    call: "Call",
    callToast: (name: string) =>
      `Calls from the dashboard are not available yet. Please phone ${name} directly.`,
    reminderToast: (name: string) => `Reminder sent to ${name}`,
    latest: "Latest report",
    nextHearing: "Next hearing",
    none: "None fixed",
    moveCases: "Move their cases",
  },
  reports: {
    title: "Reports",
    description:
      "How the office is doing: how many people asked for help, what kind of help, and how cases ended.",
    download: "Download report",
    kpi: {
      newThisMonth: "New cases this month",
      newDelta: (n: string) => `${n} compared with last month`,
      closedYear: "Cases closed this year",
      closedHint: "All types of case",
      firstContact: "Average time to first contact",
      days: (n: string) => `${n} days`,
      firstContactDelta: (n: string) => `${n} days faster than last quarter`,
      overrides: "AI priority changed by officers",
      overridesHint: (n: string, total: string) => `${n} of ${total} AI suggestions this year`,
    },
    perMonth: {
      title: "New cases per month",
      hint: "Applications received each month, all types together.",
      thisMonth: "this month so far",
      tooltip: (n: string) => `${n} new cases`,
    },
    byType: {
      title: "Cases by type",
      hint: "All cases this year, the most common first.",
    },
    outcomes: {
      title: "How closed cases ended",
      hint: (n: string) =>
        `${n} cases closed this year. Most were decided in court or settled in mediation.`,
    },
    showTable: "Show the numbers as a table",
    table: {
      month: "Month",
      type: "Type of case",
      outcome: "Outcome",
      cases: "Cases",
      share: "Share",
    },
  },
  profile: {
    title: "My profile",
    description:
      "Your details as colleagues and case records see them. You can change how people reach you.",
    officerId: "Officer ID",
    office: "Office",
    since: (date: string) => `Officer since ${date}`,
    sensitiveAccess:
      "Authorized receiving DLAO (Role B6): you can open the evidence in sensitive cases.",
    contact: "Contact details",
    contactHint: "Used for case alerts and by the district office.",
    email: "Email",
    phone: "Mobile number",
    phoneHint: "11 digits, used for SMS alerts. Example: 01711-000142",
    save: "Save changes",
    saved: "Your contact details were saved",
    errors: {
      email: "Enter a valid email address, like name@example.org.",
      phone: "Enter an 11-digit mobile number that starts with 01.",
    },
    work: "Your work in this session",
    workHint: "Everything you decide on a case is counted here and saved in the case history.",
    decisions: "Decisions made",
    overrides: "Priorities you changed",
    reminders: "Reminders sent",
    recent: "Recent activity",
    noRecent: "Nothing yet. Decisions you make on cases will appear here.",
    security: "Sign-in and security",
    securityHint: "Keep your account safe. Never share your password, even with colleagues.",
    changePassword: "Change password",
    signOut: "Sign out of this computer",
  },
  settings: {
    title: "Settings",
    description: "Make the dashboard comfortable for you. Changes are saved on this computer.",
    language: "Language",
    languageHint: "The language for menus, buttons and case information.",
    textSize: "Text size",
    textSizeHint: "Make all text bigger if it is hard to read.",
    sizes: { standard: "Standard", large: "Large", xlarge: "Extra large" },
    preview: "Preview",
    previewText: "Moyuri Akter — do not call now. Safe time: Tuesday, 2 to 4 PM.",
    notifications: "Notifications",
    notificationsHint: "How the system tells you about important changes.",
    notify: {
      dailyEmail: {
        label: "Daily summary by email",
        hint: "Every morning at 8:00, a short list of today's work.",
      },
      urgentSms: {
        label: "SMS for critical cases",
        hint: "A text message as soon as a case becomes Critical.",
      },
      hearingReminder: {
        label: "Hearing reminders",
        hint: "A reminder the day before each hearing.",
      },
    },
    reset: "Reset to default settings",
    resetDone: "Settings are back to their defaults",
  },
  help: {
    title: "Help and guidelines",
    description: "Short answers to common questions, and what the words in this system mean.",
    startTitle: "Getting started",
    steps: [
      {
        title: "Start on Home",
        body: "It shows the few cases that need you most, in order, and what is coming up.",
      },
      {
        title: "Work through your queue",
        body: "Every case has one button for its next step. Read the reason, then press it.",
      },
      {
        title: "Every decision is recorded",
        body: "Accepting, changing or confirming anything is saved in the case history with your name.",
      },
    ],
    faqTitle: "Common questions",
    faq: [
      {
        q: "What does the AI suggestion do?",
        a: "The system reads each application and looks for warning signs, such as violence, children at risk, or someone else reporting for the applicant. It suggests how urgent the case is. It never decides: you accept the suggestion or change it.",
      },
      {
        q: "When should I override the priority?",
        a: "When you know something the application did not say. For example, the person who reported the case called again about a new threat. Write what you learned; your reason is saved with your name.",
      },
      {
        q: "What does “Do not call now” mean?",
        a: "Some applicants' phones are watched by the person harming them. Only call inside the safe time shown on the case. The Call button stays locked outside it. Never send an SMS or leave a voicemail.",
      },
      {
        q: "What do “advice”, “mediation” and “sensitive” mean?",
        a: "The AI marks every new case with how it could be resolved: through advice alone, through mediation with the other side, or as a sensitive case (violence, threats or another risk). It is only a mark. Confirm it or change it in the AI suggestion tab; your decision is saved with your name.",
      },
      {
        q: "What does “Do not call this number” mean?",
        a: "The caller said something suggesting they are being held, or the call was cut while they were describing violence. A ringing phone or an SMS could alert the person with them, so calls and texts are blocked. Wait for them to call the helpline again, or work with the police.",
      },
      {
        q: "Why can't I merge two records that look the same?",
        a: "Families often share one phone and live in the same village, so two applications can look alike. If the National ID numbers are different, they are different people, and the system blocks merging to protect both.",
      },
      {
        q: "What should I do when a lawyer stops reporting?",
        a: "Panel lawyers send a progress update every two weeks. After two missed updates the case shows an alert. Send a reminder first; if nothing changes, assign another lawyer.",
      },
      {
        q: "How do lawyers report from court?",
        a: "Panel lawyers post what happened at each hearing, and the next date, from their own dashboard. It appears in the case's Court progress tab and on Hearings. A report is due every 2 weeks and within 3 days of each hearing; a lawyer who misses them is flagged.",
      },
      {
        q: "What if other offices keep sending a case back?",
        a: "Every transfer is recorded in the case. After a case has been sent back twice, press Escalate to Chief Officer: the Chief Legal Aid Officer decides who handles it, and every office must follow that decision.",
      },
      {
        q: "Why must some cases be transferred?",
        a: "A district office can only act on cases inside its area. If the accused lives elsewhere, or the case needs a special court, it is sent to the right office through the national legal aid organisation. You keep supporting the applicant.",
      },
    ],
    priorityTitle: "What the priority levels mean",
    priorityExample: {
      critical: "A threat to life was reported.",
      high: "Ongoing violence, or someone who cannot safely contact us.",
      medium: "A deadline or money problem, with no danger.",
      low: "Both sides agree to talk; no urgency.",
    },
    glossaryTitle: "Words used in this system",
    glossary: [
      {
        term: "DLAO",
        meaning: "District Legal Aid Officer — the officer who runs legal aid in the district.",
      },
      {
        term: "Panel lawyer",
        meaning: "A private lawyer the district pays to take legal aid cases.",
      },
      {
        term: "Proxy report",
        meaning:
          "When someone else, like a neighbour, tells us about a person who cannot contact us safely.",
      },
      {
        term: "Safe contact window",
        meaning: "The only time it is safe to call an applicant whose phone is watched.",
      },
      {
        term: "AI triage",
        meaning: "The system's suggestion of how urgent a new case is, with the reasons it found.",
      },
      {
        term: "Fuzzy match",
        meaning:
          "How similar two records look, even with spelling differences. It is a hint, not proof.",
      },
      {
        term: "Mediation",
        meaning:
          "A meeting run by this office where both sides try to settle without going to court.",
      },
      {
        term: "Jurisdiction",
        meaning: "The area or court that is allowed to handle a case.",
      },
      {
        term: "Receiving DLAO (Role B6)",
        meaning:
          "The officer allowed to open the evidence in a sensitive case and confirm it arrived.",
      },
      {
        term: "Pattern alert",
        meaning:
          "A warning that one lawyer has missed several reports across their cases, not just one.",
      },
    ],
    contactTitle: "Need more help?",
    contacts: {
      helpline: "National legal aid helpline",
      helplineHint: "Free to call, for applicants and officers",
      desk: "District office helpdesk",
      deskHint: "Sunday to Thursday, 9:00 to 17:00",
      support: "System support",
      supportHint: "For problems with this dashboard",
    },
  },
  hearings: {
    title: "Hearings",
    description:
      "Court dates and mediation meetings for your cases. Make sure a lawyer and the applicant know about each one.",
    court: "Court hearing",
    mediation: "Mediation meeting",
    mediationHint: "Run by this office to settle the dispute without going to court.",
    where: "Where",
    lawyer: "Lawyer",
    noLawyer: "No lawyer needed",
    none: "No hearings in the next two weeks.",
    openCase: "Open case",
    remind: "Send reminder",
    reminded: "Reminder sent",
    remindHint: "Texts the lawyer and applicant the date, time and place.",
    remindedToast: (name: string) => `Reminder sent for ${name}'s hearing`,
    summary: (n: string) => `${count(n, "hearing", "hearings")} in the next two weeks`,
    afterStage: (stage: string) => `Next date after: ${stage}`,
    mode: {
      in_person: "Mediation room, District Legal Aid Office",
      odr_video: "Video call",
      odr_phone: "Phone call",
    },
    reported: "Date reported by the lawyer",
  },
  queue: {
    title: "Work queue",
    description: "Cases waiting for your decision. The most urgent ones are at the top.",
    filterLabel: "Which cases to show",
    all: "All open cases",
    actionToday: "Needs Action Today",
    pendingTriage: "Pending AI Triage",
    duplicates: "Duplicates for Review",
    alerts: "Overdue / Alerts",
    hint: {
      all: "Every case that is still open.",
      actionToday: "Cases with a deadline or a safety risk today. Start here.",
      pendingTriage:
        "The system has suggested how urgent these cases are. Check each suggestion, then accept it or change it.",
      duplicates:
        "New applications that look like someone who applied before. Compare them and decide if it is the same person.",
      alerts:
        "Late work, lawyers who stopped reporting, and cases that must move to another office.",
    },
    searchLabel: "Search cases",
    searchPlaceholder: "Name, case or tracking number, or place",
    backlog: {
      label: "Backlog right now",
      new: (n: string) => `${n} New`,
      urgent: (n: string) => `${n} Urgent`,
      overdue: (n: string) => `${n} Overdue`,
      hint: {
        new: "Received in the last 24 hours",
        urgent: "High or critical priority",
        overdue: "Late tasks and late lawyer reports",
      },
    },
    concernFilter: "Show only",
    concern: {
      any: "All kinds of case",
      children: "Children at risk",
      proxy: "Reported by someone else",
      sensitive: "Sensitive cases",
    },
    priorityFilter: "Filter by priority",
    anyPriority: "Any priority",
    listLabel: "Cases, most urgent first",
    showing: (n: string, total: string) => `Showing ${n} of ${total}`,
    empty: "Nothing here",
    emptyHint: "No cases match. Try another filter or clear the search.",
    received: (rel: string) => `Received ${rel}`,
    due: (rel: string) => `Due ${rel}`,
    overdue: (rel: string) => `Overdue — was due ${rel}`,
    actionFor: (action: string, name: string) => `${action}: ${name}`,
    sensitive: "Sensitive case — details hidden",
    reasonLabel: "Why it is here",
  },
  stats: {
    label: "Summary",
    open: "Open cases",
    openHint: "All cases still in progress",
    urgent: "High or critical",
    urgentHint: "Need a response today",
    restricted: "Contact restricted",
    restrictedHint: "Only call in the safe time",
    alerts: "Alerts",
    alertsHint: "Late work or transfers",
  },
  priority: {
    critical: "Critical",
    high: "High",
    medium: "Medium",
    low: "Low",
    label: (p: string) => `Priority: ${p}`,
    meaning: {
      critical: "Act within hours",
      high: "Act today",
      medium: "Act this week",
      low: "When possible",
    },
    overridden: "Changed by officer",
  },
  category: {
    domesticViolence: "Domestic Violence",
    cyberHarassment: "Cyber Harassment",
    landDispute: "Land Dispute",
    familyMaintenance: "Family Maintenance",
    dowryHarassment: "Dowry Harassment",
    labourDispute: "Labour / Wages",
    childCustody: "Child Custody",
    criminalDefence: "Criminal defence",
    other: "Not sorted yet",
  },
  flag: {
    proxyReported: "Proxy Reported",
    restrictedContact: "Restricted window",
    lawyerInactivity: "Lawyer Inactivity Alert",
    sensitive: "Sensitive",
    jurisdictionEscalation: "Jurisdiction Escalation",
    possibleDuplicate: "Possible duplicate",
    overdue: "Overdue",
    escalated: "Transferred",
    doNotCall: "Do not call this number",
    callDropped: "Call was cut",
    inCustody: "In custody",
    mediationNoShow: "Missed mediation",
    proxyBy: (name: string) => `Proxy Reported by ${name}`,
    lawyerMissed: (n: string) => `Lawyer Inactivity: missed ${count(n, "update", "updates")}`,
  },
  action: {
    reviewTriage: "Review AI suggestion",
    reviewDuplicate: "Compare records",
    followUpLawyer: "Remind lawyer",
    escalateJurisdiction: "Transfer case",
    assignLawyer: "Assign a lawyer",
    resolveOverdue: "Resolve late task",
    scheduleSafeCall: "Book a safe call",
    viewCase: "Open case",
  },
  /** One plain sentence per next action: why the case is in the queue. */
  reason: {
    reviewTriage: (p: string) => `The AI suggests ${p} priority. It needs your check.`,
    reviewDuplicate: (id: string) => `May be the same person as ${id}.`,
    followUpLawyer: (n: string) =>
      `The lawyer has missed ${count(n, "progress update", "progress updates")}.`,
    escalateJurisdiction:
      "The accused lives outside this district, so the case must be transferred.",
    returned: (n: string) =>
      `Other offices sent it back ${n} times. The Chief Legal Aid Officer must decide.`,
    resolveOverdue: (task: string) => `${task} is late.`,
    assignLawyer: "No lawyer has been assigned yet.",
    scheduleSafeCall: "Book a call with the applicant in her next safe time.",
    viewCase: "Nothing to do right now.",
  },
  channel: {
    hotline: "Hotline 16430",
    walkIn: "Came to the office",
    online: "Online form",
    proxy: "Reported by someone else (proxy)",
    udc: "Union Digital Centre",
    court: "Court",
    prison: "Jail",
  },
  safety: {
    doNotCall: "DO NOT CALL NOW",
    doNotCallShort: "Do not call now",
    safeNow: "Safe to call now",
    window: (w: string) => `Safe Contact Window: ${w}`,
    short: (w: string) => `Safe time to call: ${w}`,
    explain:
      "Her phone is watched by the person harming her. Call only in the safe time. Never send an SMS or leave a voicemail.",
    nextWindow: (when: string) => `Next safe time: ${when}`,
    closesAt: (time: string) => `Safe time ends at ${time}`,
    call: "Call applicant",
    callBlocked: "Calling is blocked until the safe time.",
  },
  detail: {
    caseLine: (id: string, category: string) => `Case ${id} · ${category}`,
    tabsLabel: "Case sections",
    tabTriage: "AI suggestion",
    tabDetails: "Case information",
    tabCourt: "Court progress",
    tabRecords: "Court and jail records",
    tabMediation: "Mediation",
    tabActivity: "History",
    summary: "What happened",
    applicant: "About the applicant",
    name: "Name",
    phone: "Phone",
    village: "Village",
    guardian: "Father / husband",
    nid: "National ID",
    age: "Age",
    category: "Type of case",
    channel: "How it was reported",
    received: "Received",
    filedHow: "Who filed it",
    identity: "National ID check",
    trackingToken: "Tracking number",
    lawyer: "Lawyer",
    unassigned: "Not assigned yet",
    sensitiveNotice: "Sensitive case. Her identity is protected and every view is recorded.",
    whatToDo: "What to do now",
    noActions: "Nothing to do on this case right now.",
    close: "Close",
  },
  triage: {
    title: "AI Triage Recommendation",
    explain:
      "The system read the application and looked for warning signs. Ticked items were found. The final decision is yours.",
    recommended: "Recommended priority",
    confidence: (p: string) => `How sure the AI is: ${p}`,
    checksFound: (n: string, total: string) => `${n} of ${total} AI checks found warning signs`,
    factors: "Warning signs",
    detected: "Found",
    notDetected: "Not found",
    weight: {
      high: "very important",
      medium: "important",
      low: "minor",
    },
    rationale: "Why this priority",
    decision: "Your decision",
    accept: "Accept recommendation",
    acceptHint: "Keep the suggested priority.",
    override: "Override Priority",
    overrideHint: "Choose a different priority and say why.",
    status: {
      pending: "Waiting for your decision",
      accepted: "You accepted this",
      overridden: "You changed this",
    },
    none: "The AI has not checked this case yet.",
    acceptedToast: (id: string, p: string) => `${id}: ${p} priority accepted`,
    agent: {
      intake: "Application check",
      risk: "Risk check",
      safety: "Safety check",
      jurisdiction: "Jurisdiction check",
    },
    factor: {
      activeViolence: "Active Violence Detected",
      proxyReported: "Proxy Reported (Access Barrier)",
      safeContactRestricted: "Safe Contact Restricted",
      childrenInHousehold: "Children in the household",
      weaponThreat: "Weapon or threat to life",
      hostageSituation: "Possibly held hostage",
      priorLegalAction: "Earlier legal action on record",
      onlineAbuse: "Private images shared online",
      extortionThreat: "Money demanded with threats",
      outOfJurisdiction: "Accused lives outside the district",
      financialDependency: "Depends on the other party for money",
      hearingImminent: "Court hearing within 14 days",
    },
  },
  override: {
    title: "Override Priority",
    description:
      "Pick the priority you think is right and explain why. Your note is saved in the case history with your name.",
    newPriority: "New priority",
    selectPriority: "Choose a priority",
    current: "current",
    justification: "Justification for Override",
    justificationHelp: (n: string) =>
      `Required. Write at least ${n} characters about what the AI missed.`,
    placeholder: "Example: Ripon called again. The husband threatened her with a knife last night.",
    counter: (n: string, min: string) => `${n} of ${min} characters`,
    errors: {
      priorityRequired: "Choose a new priority.",
      prioritySame: "Choose a priority that is different from the current one.",
      justificationRequired: "Please explain why you are changing the priority.",
      justificationShort: (n: string) => `Please write at least ${n} characters.`,
    },
    cancel: "Cancel",
    save: "Save new priority",
    savedToast: (id: string, p: string) => `${id}: priority changed to ${p}`,
  },
  duplicate: {
    eyebrow: "Possible duplicate record",
    title: "Is this the same person?",
    description:
      "The system found an older application that looks similar. Compare the two side by side, then decide.",
    confidence: (p: string) => `Fuzzy Match Confidence: ${p}`,
    confidenceHint: "How similar the two records look. It does not prove they are the same person.",
    matched: (n: string, total: string) => `${n} of ${total} details are the same`,
    sharedPhone:
      "Families in villages often share one phone, so a matching phone number alone does not mean it is the same person.",
    existing: "Older application",
    incoming: "New application",
    caption: "The two applications side by side",
    field: "Detail",
    match: "Same",
    conflict: "Different — merge blocked",
    decision: "Your decision",
    merge: "Merge Records",
    mergeNotAllowed: "Not allowed",
    mergeBlocked: "You cannot merge these: their National ID numbers are different.",
    confirmDistinct: "Confirm as Distinct Individuals",
    confirmHint: "They are two different people. Keep both applications.",
    distinctToast: (a: string, b: string) => `${a} and ${b} saved as two different people`,
    resolved: "Done — saved as two different people",
  },
  followUp: {
    lawyerTitle: "The lawyer has stopped reporting",
    lawyerMissed: (name: string, n: string) =>
      `${name} has missed ${count(n, "progress update", "progress updates in a row")} (they are due every 2 weeks).`,
    lawyerLast: (rel: string) => `Last update ${rel}`,
    sendReminder: "Send reminder to lawyer",
    reminderToast: (name: string) => `Reminder sent to ${name}`,
    escalateTitle: "This case must go to another office",
    escalateTo: (target: string) => `Send to: ${target}`,
    escalate: "Transfer case",
    escalatedToast: (id: string) => `${id} sent for transfer`,
    chiefTitle: "Sent back twice: ask the Chief Legal Aid Officer to decide",
    chiefBody: (n: string) =>
      `Other offices have sent this case back ${n} times. The Chief Legal Aid Officer's decision on who handles it binds every office, so the applicant stops waiting.`,
    chief: "Escalate to Chief Officer",
    chiefToast: (id: string) => `${id} escalated to the Chief Legal Aid Officer`,
    overdueTitle: "A task is late",
    overdueWas: (task: string, rel: string) => `${task} was due ${rel}.`,
    markSubmitted: "Mark as done",
    overdueToast: (id: string) => `${id}: late task marked as done`,
    assignTitle: "This case needs a lawyer",
    assignBody: "Choose a lawyer from the district panel.",
    selectLawyer: "Choose a lawyer",
    assign: "Assign",
    assignedToast: (name: string, id: string) => `${name} assigned to ${id}`,
    safeCallTitle: "Book a safe call",
    safeCallWhen: (when: string) => `We will call her in the next safe time: ${when}.`,
    scheduleCall: "Book call for the safe time",
    scheduledToast: (when: string) => `Call booked for ${when}`,
    triageTitle: "Check the AI suggestion",
    triageBody:
      "Look at the warning signs below. Then accept the suggested priority, or override it and write why.",
    duplicateTitle: "This may be a duplicate",
    duplicateBody: "Compare this application with the older one before doing anything else.",
    openDuplicate: "Compare records",
  },
  activity: {
    label: "Case history, newest first",
    system: "System",
    ai: "AI",
    justification: "Reason given",
    received: (channel: string) => `Application received (${channel})`,
    aiTriage: (p: string) => `AI suggested ${p} priority`,
    triageAccepted: (p: string) => `Officer accepted ${p} priority`,
    priorityOverride: (from: string, to: string) => `Officer changed priority: ${from} → ${to}`,
    duplicateFlagged: (id: string, score: string) =>
      `System flagged a possible duplicate of ${id} (${score} similar)`,
    duplicateDistinct: (id: string) => `Officer confirmed a different person from ${id}`,
    lawyerUpdateMissed: "Lawyer missed a progress update",
    lawyerReminder: "Reminder sent to the lawyer",
    escalated: "Sent for transfer to the national legal aid office",
    escalatedChief: "Escalated to the Chief Legal Aid Officer after other offices sent it back",
    lawyer: "Lawyer",
    lawyerUpdate: (name: string, stage: string) => `${name} reported from court: ${stage}`,
    lawyerReassigned: (from: string, to: string) => `Case moved from ${from} to ${to}`,
    evidenceViewed: "Sensitive evidence opened",
    evidenceAcknowledged: "Receipt of the sensitive evidence acknowledged",
    overdueResolved: "Late task marked as done",
    lawyerAssigned: (name: string) => `Assigned to ${name}`,
    safeCallScheduled: (when: string) => `Safe call booked for ${when}`,
    trackReviewed: (to: string) => `Officer set the way forward: ${to}`,
    doNotCallSet: (reason: string) => `Marked do-not-call. ${reason}`,
    noticeHeld: "SMS to the other side held for the officer",
    noticeReleased: "Officer sent the SMS to the other side",
    identityVerified: "Caller's identity confirmed with the National ID register",
    identityNotVerified: "Caller's identity could not be confirmed",
  },
  doNotCall: {
    title: "DO NOT CALL THIS NUMBER",
    reason: {
      hostage:
        "The victim may be in a hostage situation. A ringing phone or an SMS could alert whoever is holding them.",
      dangerCallCut:
        "The call was cut while the caller was describing violence. Calling back could alert the person harming them.",
    },
    short: {
      hostage: "Do not call: possible hostage situation",
      dangerCallCut: "Do not call: the call was cut during danger",
    },
    what: "Calls and SMS to the applicant are blocked. Wait for them to call the helpline again, or work with the police.",
    blocked: "Calling and SMS are blocked for this applicant.",
    police: "Call the police (999)",
    policeHint: "If you believe someone is in danger now, call 999 and give the case number.",
  },
  track: {
    title: "Suggested way forward",
    explain:
      "The AI marks how this case could be resolved. It does not decide: you confirm the mark or change it.",
    label: {
      advice: "Can be resolved through advice",
      mediation: "Can be resolved through mediation",
      sensitive: "Sensitive case",
    },
    hint: {
      advice: "Legal advice or information alone could settle this.",
      mediation: "A dispute with another side that mediation could settle.",
      sensitive: "Violence, threats or another risk. Handle with care.",
    },
    status: {
      suggested: "AI mark — waiting for your decision",
      confirmed: "You confirmed this",
      changed: "You changed this",
    },
    aiSaid: (label: string) => `The AI marked: ${label}`,
    why: "Why the AI marked it",
    confirm: "Confirm this mark",
    confirmHint: "Keep the AI's mark.",
    change: "Change the mark",
    changeHint: "Choose another way forward and say why.",
    newTrack: "Way forward",
    choose: "Choose a way forward",
    justification: "Why are you changing it?",
    justificationHelp: (n: string) => `Required. Write at least ${n} characters.`,
    save: "Save",
    cancel: "Cancel",
    errors: {
      trackRequired: "Choose a way forward.",
      same: "Choose a different option, or confirm the AI's mark instead.",
      short: (n: string) => `Please write at least ${n} characters.`,
    },
    confirmedToast: (id: string, label: string) => `${id}: "${label}" confirmed`,
    changedToast: (id: string, label: string) => `${id}: changed to "${label}"`,
  },
  identity: {
    filedFor: {
      self: "By the applicant, for themselves",
      father: "By the caller, for their father",
      mother: "By the caller, for their mother",
      sibling: "By the caller, for their brother or sister",
      other: "By someone else, for the applicant",
    },
    verified: "Confirmed with the National ID register",
    notVerified: "Not confirmed yet. Check the NID at the office.",
    callerVerified: "The caller answered the NID security questions",
    callerVerifiedBySim: "Confirmed by the SIM they called from, not the security questions",
    callerVerifiedBySimFamily:
      "Confirmed as a relative of the SIM's owner, not by the security questions",
    simRegistered: "Called from a SIM registered to their own NID",
    ekyc: "Identity verified by e-KYC",
    ekycAt: (office: string) => `Identity verified by e-KYC at ${office}`,
    ekycPending: "Not verified yet. The court or jail can check the NID by e-KYC.",
  },
  respondent: {
    title: "Complaint against",
    foundInNid: "Found in the National ID register",
    notFoundInNid: "Not found in the National ID register",
    notice: {
      sent: "SMS sent asking them to visit the office",
      held: "SMS held — needs your decision",
      notFound: "No registered number, so no SMS was sent",
      blocked: "SMS blocked",
      failed: "The SMS could not be delivered",
    },
    holdReason: {
      callerDidNotAgree: "the caller did not agree to it",
      doNotCall: "the applicant must not be contacted",
      sensitive: "it is a sensitive case",
      emergency: "it was reported as an emergency",
      identityNotVerified: "the caller's identity was not confirmed",
    },
    heldBecause: (reasons: string) => `Held because ${reasons}.`,
    send: "Send the SMS now",
    sendHelp: (n: string) => `Write at least ${n} characters: why is it safe to send it now?`,
    justification: "Reason",
    sentToast: (id: string) => `${id}: SMS sent to the other side`,
  },
  receipt: {
    sent: "Sent to whoever filed the case by SMS",
    held: "Not sent yet",
    notFound: "Not sent: no phone number",
    blocked: "Not sent: contact is blocked",
    failed: "The SMS could not be delivered",
    handedOver: "Given to the court or jail staff to hand to the applicant",
  },
  notes: {
    title: "What the caller said",
    hint: "Notes the AI took during the call, in the caller's words.",
    topic: {
      opening: "Start of the call",
      filing_for: "Who the application is for",
      caller_name: "Caller's name",
      father_name: "Father's name",
      permanent_district: "Permanent district",
      date_of_birth: "Date of birth",
      name: "Applicant's name",
      district: "Where they live",
      problem: "What happened",
      respondent_name: "Complaint against",
      respondent_father_name: "Their father's name",
      respondent_district: "Their district",
      notify_respondent: "SMS to the other side",
      phone: "Phone number",
      safe_to_call: "Safe time to call",
    },
  },
  courtStage: {
    plaintFiled: "Case filed (plaint and vakalatnama)",
    evidenceRecorded: "Witness evidence recorded",
    hearingAdjourned: "Heard; order reserved or date moved",
    bailHeard: "Bail hearing held",
    settlementFiled: "Settlement filed with the court",
    judgment: "Judgment given",
    other: "Other progress",
  },
  court: {
    title: "Court progress",
    hint: "The panel lawyer reports from their own dashboard: every 2 weeks, and within 3 days of each hearing.",
    noLawyer: "No lawyer yet. Reports from court appear here once a lawyer is assigned.",
    nextHearing: "Next hearing",
    noHearing: "No date fixed",
    hearingPassed: (date: string) => `Hearing on ${date}: waiting for the lawyer's report`,
    stage: "Where the case stands",
    notStarted: "Not in court yet",
    reports: "Reports from court",
    none: "No reports yet.",
    lastReport: (rel: string) => `Last report ${rel}`,
    dueBy: (when: string) => `Next report due ${when}`,
    late: (n: string, when: string) =>
      `${count(n, "report", "reports")} missed. The last one was due ${when}.`,
    reminded: "Reminder sent: waiting for the lawyer's report.",
    from: (name: string) => `From ${name}`,
    heldOn: (date: string) => `Hearing held ${date}`,
    nextFixed: (when: string) => `Next date fixed: ${when}`,
    attachment: (name: string) => `Attached: ${name}`,
  },
  provenance: {
    title: "Who reported it, and who it is about",
    hint: "Someone may report for a person who cannot reach us safely. Keep who spoke apart from who the case is about.",
    caller: "Who reported it",
    subject: "Who the case is about",
    self: "The applicant reported it themselves.",
    confirmed: "Identity confirmed",
    notConfirmed: "Identity not confirmed",
    age: (n: string) => `Age ${n}`,
    consentYes: "The applicant agreed to this report being made for them.",
    consentNo:
      "The applicant's agreement is not recorded yet. Confirm it with them, safely, before acting.",
  },
  submitted: {
    by: (office: string, staff: string) => `Submitted by ${office} · ${staff}`,
    forApplicant: (office: string) => `By ${office}, for the applicant`,
    hint: "Court and jail staff apply for someone in front of them, who may have no phone. Reach the applicant through that office.",
  },
  referral: {
    title: "Transfers between offices",
    hint: "Each time the case was sent to another office, and what that office did.",
    sentBack: (n: string) => `Sent back ${n === "1" ? "once" : `${n} times`}`,
    office: (name: string) => `Legal aid office, ${name}`,
    hop: (from: string, to: string) => `From ${from} to ${to}`,
    status: {
      pending: "Waiting for an answer",
      accepted: "Accepted",
      returned: "Sent back",
      escalated: "Stopped: needs a decision above district level",
    },
    why: "Why it was sent",
    answer: "Their answer",
    answered: (date: string) => `Answered ${date}`,
    escalated: "Escalated to the Chief Legal Aid Officer. Their decision binds every office.",
  },
  evidence: {
    title: "Documents and evidence",
    hint: "Files sent with the application, and court orders from the lawyer.",
    restricted: "Access Restricted - Viewable only by Authorized Receiving DLAO (Role B6).",
    authorized:
      "You are the authorized receiving DLAO (Role B6). Opening the files is recorded in the case history.",
    show: "Show the files",
    hide: "Hide the files",
    file: (n: string) => `File ${n}`,
    type: { image: "Image", pdf: "PDF document", text: "Text" },
    signature: "Applicant's e-signature",
    uploaded: (when: string) => `Uploaded ${when}`,
    fingerprint: (hash: string) => `SHA-256 fingerprint ${hash}…`,
    waiting: "Waiting for the receiving officer to acknowledge receipt.",
    sentBy: (office: string) => `Sent by ${office}.`,
    acknowledged: (name: string, when: string) =>
      `Receipt acknowledged by ${name}, ${when}. The sending office sees this.`,
    acknowledge: "Acknowledge Receipt",
    openFailed: "The files could not be opened. Please try again.",
    acknowledgedToast: (id: string) => `${id}: receipt of the evidence acknowledged`,
  },
  records: {
    hint: "What the courts and jails hold on this applicant. Every look is recorded in the audit log.",
    loading: "Loading the court and jail records…",
    error: "The court and jail records could not be loaded.",
    retry: "Try again",
    empty: "No court or jail records are linked to this case",
    emptyHint: "Link the applicant's court case or prisoner record to see their hearings here.",
    link: "Link a record",
    submission: "Who submitted it, and how the applicant was identified",
    submittedAt: (when: string) => `Submitted ${when}`,
    helpNeeded: (help: string) => `Help asked for: ${help}`,
    help: {
      defence: "defence in court",
      bail: "bail",
      appeal: "an appeal",
      family: "a family case",
      civil: "a civil case",
      other: "other legal help",
    },
    inCustody: "The applicant is in custody.",
    ekyc: "e-KYC (National ID check)",
    ekycStatus: {
      verified: "Verified with the National ID register",
      notMatched: "The details did not match the register",
      unavailable: "The register could not be reached",
    },
    ekycBy: (by: string, when: string) => `Checked by ${by}, ${when}`,
    noEkyc: "Not checked yet. The court or jail can check the NID by e-KYC.",
    nid: (last4: string) => `NID •••• ${last4}`,
    signature: "Applicant's e-signature",
    signedBy: (by: string, when: string) => `Signed in front of ${by}, ${when}`,
    noSignature: "Not signed yet. Staff add it after the e-KYC check.",
    fingerprint: (hash: string) => `SHA-256 fingerprint ${hash}…`,
    caseType: {
      criminal: "Criminal case",
      civil: "Civil case",
      family: "Family case",
      womenChildren: "Women and children case",
      labour: "Labour case",
      other: "Other case",
    },
    status: { pending: "Pending", disposed: "Disposed" },
    restricted: "Restricted by the court: handle with care",
    filed: (date: string) => `Filed ${date}`,
    next: "Next date",
    noNext: "No date fixed",
    parties: "Parties",
    role: {
      accused: "Accused",
      complainant: "Complainant",
      petitioner: "Petitioner",
      respondent: "Respondent",
      plaintiff: "Plaintiff",
      defendant: "Defendant",
      witness: "Witness",
    },
    father: (name: string) => `father ${name}`,
    age: (n: string) => `age ${n}`,
    proceedings: "What happened in court",
    noProceedings: "Nothing recorded by the court yet.",
    kind: {
      hearing: "Hearing",
      chargeFraming: "Charge framed",
      evidence: "Evidence",
      bail: "Bail hearing",
      argument: "Arguments",
      order: "Order",
      judgment: "Judgment",
      other: "Other",
    },
    nextFixed: (date: string, purpose: string) => `Next date: ${date}, ${purpose}`,
    lawyers: "Lawyers",
    noLawyers: "No lawyer on record. This person may have nobody to defend them.",
    side: {
      defence: "defence",
      prosecution: "prosecution",
      plaintiff: "plaintiff",
      defendant: "defendant",
      petitioner: "petitioner",
      respondent: "respondent",
    },
    current: "On the case now",
    former: "No longer on the case",
    period: (from: string, until: string) => `${from} to ${until}`,
    since: (from: string) => `Since ${from}`,
    causeList: "Coming up on the cause list",
    causeListHint: "The court's list of cases it will hear each day.",
    slot: (serial: string, time: string) => `Serial ${serial}, ${time}`,
    slotNoTime: (serial: string) => `Serial ${serial}`,
    noCauseList: "Not on an upcoming cause list.",
    custody: "Held in",
    prisoner: "Prisoner record",
    prisonerNo: "Prisoner no.",
    prisonerStatus: {
      undertrial: "Undertrial",
      convicted: "Convicted",
      released: "Released",
      transferred: "Transferred",
    },
    ward: "Ward",
    admitted: "Admitted",
    theirCases: "Cases they are held on",
    notRegistered: "Not registered by the court yet",
    previous: "Previous records",
    previousHint:
      "The same person's other court cases, matched by National ID or by name and father's name. Restricted records are never shown here.",
    noPrevious: "No other court cases found for this person.",
  },
  mediation: {
    hint: "Meetings this office runs so both sides can settle without going to court. Each party gets an SMS notice with its own notice number.",
    loading: "Loading mediation…",
    error: "Mediation could not be loaded.",
    retry: "Try again",
    schedule: "Schedule mediation",
    sessions: "Sessions",
    none: "No mediation sessions yet.",
    missedTitle: "Missed in a row",
    missedHint: (limit: string) =>
      `After ${limit} missed sessions in a row, the party's Union Digital Centre is asked to tell them the next date in person.`,
    missed: (n: string, limit: string) => `${n} of ${limit} missed`,
    role: { applicant: "Applicant", respondent: "Other side" },
    status: { scheduled: "Scheduled", held: "Held", missed: "Missed", cancelled: "Cancelled" },
    mode: { in_person: "In person", odr_phone: "By phone", odr_video: "Video call" },
    minutes: (n: string) => `${n} minutes`,
    joinLink: "Open the meeting link",
    attendance: "Who came",
    present: "Present",
    absent: "Absent",
    notRecorded: "Not recorded yet",
    notices: "SMS notices",
    noNotices: "No SMS notice was sent for this session.",
    notice: {
      sent: "SMS sent",
      held: "Held, not sent",
      blocked: "Blocked by the contact rules",
      failed: "The SMS could not be delivered",
      notFound: "No number to send to",
    },
    noticeNo: (code: string) => `Notice no. ${code}`,
    testMode: "test mode (not sent)",
    heldBecause: (reasons: string) => `Held because ${reasons}.`,
    reason: {
      doNotCall: "nobody may contact the applicant",
      sensitive: "it is a sensitive case",
      applicantSafety: "the applicant's safety comes first",
      other: "a safety rule stopped it",
    },
    record: "Record attendance",
    change: "Change attendance",
    udc: {
      title: "Union Digital Centres asked to reach someone",
      hint: "The centre is sent the next date and place to tell the person in person. It is never told what the case is about.",
      party: (role: string, name: string) => `${role}: ${name}`,
      father: (name: string) => `father ${name}`,
      missed: (n: string) => `Missed ${count(n, "session", "sessions")} in a row`,
      centre: (udc: string, person: string) => `${udc}, run by ${person}`,
      noCentre: "No Union Digital Centre is listed for their upazila.",
      forSession: (when: string, place: string) => `To pass on: ${when}, ${place}`,
      status: {
        sent: "Sent to the centre",
        held: "Held for your decision",
        noUdc: "Not sent: no centre for their upazila",
        failed: "The SMS to the centre failed",
        informed: "The centre told them in person",
      },
      informed: (when: string) => `Told them ${when}`,
      heldExplain:
        "Held to keep the applicant safe: telling a centre where they live could put them at risk. Send it only once you are sure it is safe, for example after speaking to them at a safe time.",
      release: "Send to the centre",
      justification: "Why is it safe to send it now?",
      justificationHelp: (n: string) => `Required. Write at least ${n} characters.`,
      short: (n: string) => `Please write at least ${n} characters.`,
      cancel: "Cancel",
      releasedToast: (udc: string) => `Sent to ${udc}`,
      failed: "That could not be sent. Please try again.",
    },
  },
  mediationForm: {
    title: "Schedule mediation",
    description: (id: string) => `A meeting for case ${id}, run by this office.`,
    date: "Date",
    time: "Time",
    timeHint: (offset: string) => `Office time (${offset}).`,
    duration: "How long",
    minutes: (n: string) => `${n} minutes`,
    mode: "How they meet",
    modeHint: {
      in_person: "At the District Legal Aid Office",
      odr_phone: "The office calls both sides",
      odr_video: "Both sides join a video call",
    },
    meetingUrl: "Meeting link",
    meetingUrlHint: "The video call link. It is not put in the SMS; the office sends it.",
    notes: "Notes for the file",
    notesHint: "For the office only. Not sent to anyone.",
    notify: "Send the notice to both parties by SMS",
    notifyExplain: (helpline: string) =>
      `The SMS gives the case number, the date, time and place, and a notice number. It also gives the helpline number, ${helpline}: whoever calls it and says the notice number hears what the notice is about, where to come, and what to do if they cannot.`,
    heldSensitive: "This is a sensitive case: the SMS notices will be held for you, not sent.",
    heldDoNotCall:
      "Nobody may call or text this applicant: the SMS notices will be held, not sent.",
    outsideWindow: (windows: string) =>
      `This time is outside the applicant's safe time to be contacted (${windows}). Choose a time inside it, or meet in person.`,
    errors: {
      date: "Choose a date.",
      time: "Choose a time.",
      past: "Choose a time that has not passed yet.",
      url: "Enter a link that starts with https://",
    },
    cancel: "Cancel",
    submit: "Schedule",
    failed: "The session could not be scheduled. Please try again.",
    scheduledToast: (when: string) => `Mediation scheduled for ${when}`,
  },
  attendanceForm: {
    title: "Record attendance",
    description: (when: string) => `The session of ${when}. Mark who came.`,
    explain: (limit: string) =>
      `If someone misses ${limit} sessions in a row, their Union Digital Centre is asked to tell them the next date in person.`,
    notes: "Notes",
    notesHint: "For example, why someone could not come.",
    choose: "Choose present or absent.",
    cancel: "Cancel",
    save: "Save attendance",
    savedToast: "Attendance saved",
    failed: "Attendance could not be saved. Please try again.",
  },
  recordLink: {
    title: "Link a court or jail record",
    description:
      "Find the applicant's court case or prisoner record, then link it. Searching and linking are recorded in the audit log.",
    search: "Case number, name or prisoner number",
    hint: (n: string) => `Type at least ${n} characters.`,
    searching: "Searching…",
    none: "Nothing found. Check the spelling, or try the case number.",
    courtCases: "Court cases",
    prisoners: "Prisoners",
    link: "Link",
    linked: "Linked",
    linkLabel: (what: string) => `Link ${what}`,
    linkedToast: (what: string, id: string) => `${what} linked to ${id}`,
    failed: "That could not be linked. Please try again.",
    error: "The search did not work. Please try again.",
    found: (n: string) => `${count(n, "record", "records")} found`,
  },
  pattern: {
    eyebrow: "Pattern alert: lawyer inactivity",
    warning: (missed: string, cases: string) =>
      `Inactivity Threshold Reached: Missed ${count(missed, "update", "updates")} across ${count(cases, "case", "cases")}.`,
    body: "Lawyers report every 2 weeks and within 3 days of each hearing. Without these reports, nobody can tell the applicants what is happening in court.",
    review: "Review & Reassign",
    title: "Move this lawyer's cases",
    description: (name: string) =>
      `${name} has stopped reporting. Choose the cases to move and the lawyer who takes them. Each move is saved in the case history with your name.`,
    cases: "Cases to move",
    late: (n: string) => `Missed ${count(n, "update", "updates")}`,
    upToDate: "Up to date",
    nextHearing: (when: string) => `Next hearing ${when}`,
    newLawyer: "New lawyer",
    choose: "Choose a lawyer",
    option: (name: string, n: string) => `${name} (${count(n, "open case", "open cases")})`,
    errors: {
      cases: "Choose at least one case.",
      lawyer: "Choose the lawyer who will take the cases.",
    },
    move: (n: string) => `Move ${count(n, "case", "cases")}`,
    cancel: "Cancel",
    movedToast: (n: string, from: string, to: string) =>
      `${count(n, "case", "cases")} moved from ${from} to ${to}`,
  },
  server: {
    loading: "Loading cases…",
    error: "Could not load cases from the server.",
    retry: "Try again",
    saveFailed: "That could not be saved on the server. The case list has been reloaded.",
  },
}

export type Messages = typeof en
